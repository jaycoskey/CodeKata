...
class OskarsMaze:
    ...
    @staticmethod
    def _get_graph(xyz, do_remove_nonbranching_nodes=True):
        def tup_sum(a, b):
    ...
    def __init__(self, do_remove_nonbranching_nodes=True):
    ...
    def print_paths(self, nodes):
        for i in range(len(nodes)):
            for j in range(i + 1, len(nodes)):
                from_pos = nodes[i]
                to_pos = nodes[j]
                has_path = nx.has_path(self.graph, from_pos, to_pos)
                if has_path:
                    path = nx.shortest_path(self.graph, source=from_pos, target=to_pos)
                    print(f'Path from {from_pos} to {to_pos}: Shortest length = {len(path) - 1}')
                else:
                    print(f'Path from {from_pos} to {to_pos}: None')

    def write_dot(self, path_or_filehandle):
        write_dot(self.graph, f)


if __name__ == '__main__':
    def assert_nodes_are_in_maze(maze, nodes):
        are_corners_in_maze = [(x, y, z) in maze.graph for (x, y, z) in OskarsMaze.get_corners()]
